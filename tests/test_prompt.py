...
def test_prompt_includes_template_sections_in_order():
    text = render_prompt(goals_this_year="", goals_next_year="",
...
def test_prompt_includes_goals_when_provided():
    text = render_prompt(goals_this_year="Get promoted to Associate",
...
def test_prompt_is_reusable_with_no_hardcoded_name_or_dates():
    text = render_prompt(goals_this_year="", goals_next_year="", digest_filename="digest.md")
    assert "octo" not in text.lower()
    assert "florentiana" not in text.lower()
    assert "last 6 months" not in text.lower()
